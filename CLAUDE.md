...
- **`videoTrimMs` is anchored on the video file's `birthtime`** (recording starts at page creation, and Playwright creates the file then). The constructor timestamp is only the fallback (no video / no birthtime). Anchoring on the constructor under-trims by whatever the test did before `new Tutorial()` (goto, login, waits) and leaves every frame lagging the audio clips by that constant gap — invisible in the old serial model, glaring with narration/action overlap.
