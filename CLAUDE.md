...
- **Demo site on GitHub Pages** (`.github/workflows/deploy-site.yml`): every push to `main` re-records the e2e suite in tutorial mode and publishes to the `gh-pages` branch — `landing/index.html` (self-contained static marketing page, no build step) at the site root, the generated gallery under `/gallery/`. Narration comes from the edge-tts fallback (`pipx install edge-tts`; unlike `test.yml`, no silent `TUTORIAL_TTS_CMD` is set, so the public videos have real voice). The committed `tutorial-site.config.js` sets `baseUrl` to the full URL including `/gallery/` — scaffold derives Astro `site` + `base` from it, which is what makes sub-path hosting work. `.gitignore` no longer ignores that config file.
- **In-app widget** (`templates/site/public/widget.js` + `src/site/embed.ts`): the built gallery serves `widget.js` at the site root and one payload per tutorial at `embed/<id>.json` (plus `embed/index.json` listing slugs), written by `scaffold()` via `buildEmbedFiles()`. A consuming app loads the script (base URL derived from `document.currentScript.src`) and marks elements `data-tutorial="<slug>"`; a delegated click listener opens a shadow-DOM overlay with the video, description, and step guide (numbers precomputed in the payload — sequential, contexts don't count), linking to the full gallery page. `window.PwTutorial.open/close` is the programmatic API. Theming: payload `accent` (= site `primaryColor`) sets `--pw-tutorial-accent` inline on the host, overridable from the page via `.pw-tutorial-widget { --pw-tutorial-accent: … }`. Fetching `embed/*.json` cross-origin needs CORS on the gallery host (GitHub Pages sends `*`). RTL langs (`ar`, `he`, `fa`, `ur`) flip the panel direction.
