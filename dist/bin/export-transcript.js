...
function runApply(mdPaths) {
    ...
    for (const mdPath of targets) {
        ...
        if (report.applied.length === 0 && report.manual.length === 0)
            continue;
        console.log(`\n${name}`);
        ...
        for (const problem of report.manual) {
            ...
            if (problem.from)
                console.log(`      wanted: "${problem.from}" → "${problem.to}"`);
        }
...
//# sourceMappingURL=export-transcript.js.map