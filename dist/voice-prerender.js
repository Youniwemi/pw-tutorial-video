...
function collectFromTranscripts(transcriptDir) {
    ...
    for (const file of readdirSync(transcriptDir).filter((f) => f.endsWith('.md'))) {
        try {
            ...
            for (const entry of parseTranscript(markdown)) {
                if (entry.text.trim())
                    entries.push({ lang, text: entry.text });
            }
        }
        catch {
...
export function collectVoices(sources = {}) {
    const timelineDir = sources.timelineDir ?? process.env.TUTORIAL_OUTPUT_DIR ?? join(process.cwd(), 'tutorials', 'output');
    const transcriptDir = sources.transcriptDir ??
 ...
 */
export async function regenVoices(audioDir, options = {}) {
    ...
    console.log(`[Voice] ${all.length} narration(s) known, ${todo.length} to synthesize` +
    ...
    await pool(todo, options.workers ?? 2, async ({ lang, text }) => {
        ...
        try {
            ...
            result.done++;
            console.log(`  ✔ [${result.done}/${todo.length}] ${lang} ${text.slice(0, 60)}${text.length > 60 ? '…' : ''}`);
        }
        catch (error) {
            result.failed++;
...
//# sourceMappingURL=voice-prerender.js.map