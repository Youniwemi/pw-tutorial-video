 ...
 */
export function buildTranscriptMarkdown(data: TimelineData): string {
	const lines: string[] = [];
 ...
 */
export function parseTranscript(markdown: string): TranscriptEntry[] {
	...
	for (const rawLine of markdown.split('\n')) {
		...
		if (keyLine) {
			if (current) current.key = keyLine[1];
			continue;
		}
...
export class TranscriptCorrections {
	...
	next(kind: TranscriptEntry['kind'], key?: string): string | undefined {
		...
		if (key) {
			...
			if (keyless && keyless.length > 0) return keyless.shift();
		}
		return undefined;
	}
...
// ── Applying corrections back to the test source ──────────────────────────
...
function escapeLiteral(text: string, quote: string): string {
...
export function applyCorrections(
	...
	const report: ApplyReport = { applied: [], manual: [], unchanged: 0, files: [], timelineUpdated: false };
	...
	for (const step of data.steps) {
		...
		const change: ApplyChange = { kind, key: step.key, file: data.testFile, from: step.text, to: corrected };
			...
			if (oldSplit > 0 && newSplit > 0) {
				...
				if (oldFirst === step.key) {
					...
					if (first) {
						...
						if (second) result = { content: second.content, at: Math.max(first.at, second.at) };
					}
				}
			}
		}
